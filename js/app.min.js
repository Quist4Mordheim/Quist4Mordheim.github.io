...
function (e, t) {
	...
	e.uiRestrictSpellSelect = function () {
		if (!(e.activeProfile.restrictions.spells.length < 1)) {
			$(".panel-spells .ability-select option, .panel-spells .ability-select optgroup").hide();
			var t = "#spell-options-" + e.activeProfile.restrictions.spells[0].replace(/_/g, "-");
			$(`${t}, ${t} option`).show();
			var i = e.activeProfile.spells.map(e => `.ability-select option[value="${e.name}"]`);
			i = i.join(", "),
			$(i).hide()
		}
	},
	e.uiRestrictSkillSelect = function () {
		...
		$(`${form} option, ${form} optgroup:not(.skill-options-mixed)`).hide();
		$(skills.map(expand).join(", ")).show();
